...
/**
 * An `org` plugin owned by MEMBER_ID's organisation. user_can_install_plugin answers as the
 * database would: true for the member, false for anyone else or nobody. `published` controls
 * whether the plugins-table read finds the row, since the lookup filters on it.
 */
function stubClient(opts: { published?: boolean } = {}): { client: SupabaseClient; recorded: Recorded } {
  ...
  interface PluginChain {
    ...
    maybeSingle: () => Promise<{ data: { id: string; required_permissions: string[] } | null; error: null }>
  }
  const pluginChain: PluginChain = {
    ...
    maybeSingle: () => {
      const wantsPublished = recorded.pluginFilters.some(([c, v]) => c === "published" && v === true)
      const found = published || !wantsPublished
      return Promise.resolve({ data: found ? { id: PLUGIN_UUID, required_permissions: [] } : null, error: null })
    },
  }
  ...
  const client = {
    ...
    rpc: (fn: string, args: Record<string, unknown> = {}) => {
      ...
      if (fn === "user_can_install_plugin") {
        return Promise.resolve({ data: args.p_user_id === MEMBER_ID, error: null })
      }
      // get_plugin_with_stats would resolve for auth.uid() = NULL and hide the plugin.
      if (fn === "get_plugin_with_stats") return Promise.resolve({ data: [], error: null })
      // validate_plugin_api_key and record_plugin_download: nothing to report.
      return Promise.resolve({ data: null, error: null })
    },
    from: (table: string) => ({ select: () => (table === "plugins" ? pluginChain : versionChain) }),
    storage: {
      from: (_bucket: string) => ({
        createSignedUrl: () => Promise.resolve({ data: { signedUrl: "https://storage.example.test/signed-jar" }, error: null }),
      }),
    },
...
function mountApp(client: SupabaseClient): OpenAPIHono<{ Variables: PluginStoreContext }> {
...
for (const path of ROUTES) {
  Deno.test(`${path}: an organisation member can download their organisation's plugin`, async () => {
    ...
    const res = await mountApp(client).request(path, { headers: { Authorization: `Bearer ${jwt(MEMBER_ID)}` } })

    assertEquals(res.status, 200)
    assertEquals((await res.json()).version, "1.2.1")
    // The plugin lookup did not decide visibility...
    assertEquals(recorded.rpc.some((c) => c.fn === "get_plugin_with_stats"), false)
    // ...the install predicate did, for the caller named by the token.
    const gate = recorded.rpc.find((c) => c.fn === "user_can_install_plugin")
    assertEquals(gate?.args, { p_user_id: MEMBER_ID, p_plugin_id: PLUGIN_UUID })
  })
  ...
  Deno.test(`${path}: a non-member and an anonymous caller get the same 404 as a missing plugin`, async () => {
    ...
    const outsider = await app.request(path, { headers: { Authorization: `Bearer ${jwt(OUTSIDER_ID)}` } })
  ...
  Deno.test(`${path}: an unpublished plugin stays undownloadable`, async () => {
    ...
    assertEquals(res.status, 404)
    assertEquals(await res.json(), { error: "Plugin not found" })
    assertEquals(recorded.pluginFilters, [["plugin_id", PLUGIN_ID], ["published", true]])
  })
}
